...
-(void)dealloc{
    ...
    self._mainContentView = nil;
    self._leftSideView = nil;
    self._rightSideView = nil;
...
//    }
    ...
    [self._mainContentView addGestureRecognizer:_tapGestureRec];
    _tapGestureRec.enabled = NO;
    
    _panGestureRec = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(moveViewWithGesture:)];
    [self._mainContentView addGestureRecognizer:_panGestureRec];
...
#warning 增加的方法
/**
 此处需将添加进来的视图清空，如果直接在settingViewController中直接给MainVc等赋值就会使几个视图同时存在，影响背景色
 */
    ...
    self._rightSideView = [[UIView alloc] initWithFrame:self.view.bounds];
    [self.view addSubview:self._rightSideView];
    
    self._leftSideView = [[UIView alloc] initWithFrame:self.view.bounds];
    [self.view addSubview:self._leftSideView];
    
    self._mainContentView = [[UIView alloc] initWithFrame:self.view.bounds];
    [self.view addSubview:self._mainContentView];
...
- (void)initChildControllers:(UIViewController*)leftVC rightVC:(UIViewController*)rightVC
{
    if (_canShowRight&&rightVC!=nil) {
        ...
        [self._rightSideView addSubview:rightVC.view];
    }
    if (_canShowLeft&&leftVC!=nil) {
        ...
        [self._leftSideView addSubview:leftVC.view];
    }
}
...
- (void)showContentControllerWithModel:(NSString *)className
{
    ...
    if (self._mainContentView.subviews.count > 0)
    {
        UIView *view = [self._mainContentView.subviews firstObject];
        [view removeFromSuperview];
    }
    
    controller.view.frame = self._mainContentView.frame;
    [self._mainContentView addSubview:controller.view];
...
- (void)showRightViewController
{
    ...
    [self.view sendSubviewToBack:self._leftSideView];
...
- (void)closeSideBarWithAnimate:(BOOL)bAnimate complete:(void(^)(BOOL finished))complete
{
    ...
    if (bAnimate)
    {
        [UIView animateWithDuration:self._mainContentView.transform.tx==_LeftSContentOffset?_LeftSCloseDuration:_RightSCloseDuration
                         animations:^{
                             self._mainContentView.transform = oriT;
                         }
                         completion:^(BOOL finished) {
                             _tapGestureRec.enabled = NO;
                             showingRight=NO;
...
- (void)moveViewWithGesture:(UIPanGestureRecognizer *)panGes
{
    ...
    {
        currentTranslateX = self._mainContentView.transform.tx;
    }
    if (panGes.state == UIGestureRecognizerStateChanged)
    {
        CGFloat transX = [panGes translationInView:self._mainContentView].x;
        ...
        if (transX > 0)
        {
            ...
            if (self._mainContentView.frame.origin.x < _LeftSContentOffset)
            {
                sca = 1 - (self._mainContentView.frame.origin.x/_LeftSContentOffset) * (1-_LeftSContentScale);
        ...
        else    //transX < 0
        {
            ...
            if (self._mainContentView.frame.origin.x > -_RightSContentOffset)
            {
                sca = 1 - (-self._mainContentView.frame.origin.x/_RightSContentOffset) * (1-_RightSContentScale);
            }
            else
            {
    ...
    {
        CGFloat panX = [panGes translationInView:self._mainContentView].x;
...
- (void)configureViewShadowWithDirection:(RMoveDirection)direction
{
    ...
    self._mainContentView.layer.shadowOffset = CGSizeMake(shadowW, 1.0);
    self._mainContentView.layer.shadowColor = [UIColor blackColor].CGColor;
    self._mainContentView.layer.shadowOpacity = 0.8f;
}
